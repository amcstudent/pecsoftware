...
$resource = $con->query('SELECT pj.jobOrderId, j.position, date_inserted, firm, department, contact_name, address, billing_contact, business_type, phone, 
                         ...
                         placement_date, actual_starting_date, pc.email as clientEmail, pc.id as clientId from jobOrders as j inner join pendingJobs as pj on j.id = pj.jobOrderId
...
while ($rows = $resource->fetch_assoc()) {
       ...
       ."$".$rows["placement_fee"]."$".$rows["counselor_ultimate"]."$".$rows["invoiceNo"]."$".$rows["placement_date"]."$".$rows["actual_starting_date"]."$".$rows["clientEmail"]."$".$rows["clientId"];
}
$resource->free();
$con->close();
?>